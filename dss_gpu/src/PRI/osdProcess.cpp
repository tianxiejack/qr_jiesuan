...
OSDText_Obj g_Text[OSD_TEXT_SIZE]=
{
	...
	{eWorkMode,		eOsd_Disp,	eOsd_Update,	eWhite,	eTransparent,	MAX_CONTEXT_LEN,	10+LOFFSET,		30,	0,	{0}},
...
{
#if 0
	OSDCTRL_OBJ * pCtrlObj = pOsdCtrlObj;
//	OSDText_Obj * pTextObj = pOsdCtrlObj->pTextList;
    
	SYS_LOG *pTrkInfor =NULL;

	PPVE_MSG pSendBuf = (PPVE_MSG)g_Trk_infor;			
	pSendBuf->head.SyFlag		= 0xAA;
	pSendBuf->head.eCmdType		= eCT_SystemConfigReply;
	pSendBuf->head.uiVersion	= SOFTWARE_VERSION;
	pSendBuf->head.uiRequestId  = 1;
	pSendBuf->head.uiSize		= 0;

	pTrkInfor = (SYS_LOG*)&pSendBuf->UN_MSG.data;

	while(!pSysCtrlObj->tskArry[TSK_DRAWOSD])
	{
		SDK_SEM_PEND(pCtrlObj->osdSem,SYS_FOREVER);
		SDK_SEM_RESET(pCtrlObj->osdSem);
/*
		if(pTextObj[eBoreSightXId].osdReg!=0
		 &&pTextObj[eBoreSightYId].osdReg!=0
		 &&pTextObj[eTrackStatId].osdReg !=0)
		{
		}
*/
		if(pSysCtrlObj->bTrkInfor==TRUE&&dramMsg.SysMode==TRACK_MODE){
			pTrkInfor->TBEx 	= pAvtCtrlObj->tTrk.TBEx;
			pTrkInfor->TBEy 	= pAvtCtrlObj->tTrk.TBEy;
			pTrkInfor->TrkMode  = pAvtCtrlObj->tTrk.TrkMode;
			pTrkInfor->panSpeed = dramMsg.PaneAngSpeed;
			pTrkInfor->tilSpeed = dramMsg.TitleAngSpeed;
			
			pSendBuf->head.uiSize = sizeof(PVE_MSG_HEADER)+sizeof(SYS_LOG);
//			pDataPipe->PipeSendData(Enum_TracPort,(BYTE*)pSendBuf,pSendBuf->head.uiSize);
		}
	}
#endif
}
